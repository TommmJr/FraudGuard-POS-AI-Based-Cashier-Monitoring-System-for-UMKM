...
def score_transactions(
    ...
    if df_context is not None and not df_context.empty:
        ...
        feat_out = feat[new_mask].reset_index(drop=True)
    ...
    for idx, row in df_out.iterrows():
        ...
        gap = feat_out.loc[idx, "time_gap_seconds"] if "time_gap_seconds" in feat_out.columns else 0
        
        # Aturan Tambahan: Rapid-fire (Timegap anomaly)
        # Jika transaksi berurutan dalam waktu kurang dari 10 detik (dan bukan transaksi pertama),
        # ini secara fisik mustahil untuk checkout normal dan terindikasi fraud / spam.
        if gap > 0 and gap < 10:
            df_out.at[idx, "risk_level"] = "CRITICAL"
            df_out.at[idx, "fraud_score"] = max(df_out.at[idx, "fraud_score"], 95.0)
            df_out.at[idx, "model_used"] = model_name + " + rules (rapid-fire)"
            continue # Lewati aturan lain

        if row["risk_level"] in ["HIGH", "CRITICAL"]:
            
            # Aturan 1: Transaksi bernominal kecil/sedang (< Rp 100.000) yang terjadi setelah jeda panjang 
            # (awal shift) atau merupakan transaksi pertama (gap == 0 atau >= 3600) -> Kemungkinan besar aman (LOW)
            if row["amount"] <= 100000 and (gap >= 3600 or gap == 0):
            ...
            # Aturan 2: Transaksi bernominal kecil/sedang (< Rp 100.000) dengan jeda waktu aman (gap >= 10)
            # diturunkan ke LOW agar tidak mengganggu operasional kasir.
            elif row["amount"] <= 100000 and gap >= 10:
                df_out.at[idx, "risk_level"] = "LOW"
                df_out.at[idx, "fraud_score"] = min(df_out.at[idx, "fraud_score"], 35.0)
                df_out.at[idx, "model_used"] = model_name + " + rules"
